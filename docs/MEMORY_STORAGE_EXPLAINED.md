...
2026-09-17写作要求补充：所有 Owner 的每条 L0–L4 必须带足独立理解所需的背景。AI 先写清对象、问题、条件、关键内容、结果和限制，再逐条脱离聊天/相邻记录回读；具体逐层要求及核心算法、知识等对象的用法见[记录标准](RESEARCH_RECORDING.md#每条记录必须独立可读可以正确复用)。L2 正文讲明背景→实际经过→简单结论；L3 正文讲明在何种条件下如何使用认识，以及证据、反例和边界。payload 字段齐全不能弥补正文缺失，因为 v4 阅读入口主要交付 body_markdown。

core-algorithms 和 knowledge 采用同一生成/提交逻辑，区别是归属和材料：前者必须有公司算法文档准入，后者先采用稳定身份并固定原文。AI 可将原文方法写 L1、明示过程或本次实际分析写 L2、来源主张或条件性认识写 L3、用途及边界写 L4；没有材料支持的层不编造。创建 Owner、索引原文和形成这些规范记录不是同一动作，也没有后台模型自动补全五层。
