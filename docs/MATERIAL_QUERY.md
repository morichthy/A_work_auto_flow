...
2026-09-18：Owner 文稿模式的默认召回使用独立 Owner 压缩发现索引，先从 L4/L3、L2 紧凑内容、L1 检索说明和 Owner 元数据融合 Owner，再交付覆盖互补筛选包。它不是本页现有全文索引增加 level 过滤；发现索引不可用、覆盖不完整或结果不足时，是否搜索 L1 blocks、完整文稿等全文由用户明确选择。旧 legacy 查询和已冻结旧会话仍走原兼容路径。详细契约、现行入口与验证边界见[Owner发现设计](design/OWNER_DISCOVERY_RETRIEVAL.md)。
