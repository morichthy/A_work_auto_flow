...
recent/snapshot用于快速展示最近阅读问题；其快照不替代reading-handoff的当前授权与固定来源核验。evidence search按标题、概览、结论或ID查元数据，detail按固定身份按需读取上下文、引用和影响，`metadata_only`不表示原件已重新核验。

本页面向使用者和开发 AI，用伪代码与必要说明解释核心功能，保留影响理解的选项、算法和边界。核对日期：2026-09-18（补充 Owner 压缩发现实现及验证缺口），依据当前工作树。细节按各节实现文件读取；完整字段见[文档索引](README.md)，模块关系见 [ARCHITECTURE](../ARCHITECTURE.md)。
...
工作台当前标签页保留已访问功能的结果、选择和草稿。系统记忆默认研究经过，其次对象记忆、关联导航；对象记忆由用户主动生成。证据搜索先轻量查询，选中记录后才按需读取详情；显式刷新及相关任务完成仍核对最新数据。临时视图不是持久研究记录，刷新浏览器后重建，也不会延长查询游标有效期。
...
| G06 去重与排序     | 各通道候选 → 稳定顺序与命中解释                       | 规范身份去重、RRF/主题优先；AI阅读可用正文、离线Cross-encoder及显式条件重排；排名不是科学可信度 | RET、MQ、READ  |
...
对象记忆常用筛选只显示 L0–L4 与研究文稿，章节归入文稿；目标、检查点、策略和复核等必要工作流记录按需展开。证据详情将完整正文与辅助结构字段分开，逐 claim 显示已存确认状态和内容版本匹配；保存、确认和当前证据有效性不合并。
...
新查询先从程序设置取得默认数量与预算，再由当前请求固定范围和累计上限。工作台“工作区设置”分别配置材料查询和AI阅读；显式请求优先，旧会话不随着默认设置变化而放宽预算。协作开关由工作流读取，关闭或宿主没有能力时主Agent独立工作，检索程序不需要子Agent。具体参数与生效范围见[工作区设置](WORKSPACE_SETTINGS.md)；实现由`workspace_settings.py`及CLI/HTTP适配负责。

设置页也可填写子Agent能力与模型偏好，默认使用较低能力、低成本和低推理。程序缓存后交给宿主选择实际可用模型，不另行调用AI解释配置；明确关闭协作时能力偏好不会触发委派。旧设置缺新字段时补默认，已保存的自定义配置升级保留。

### Owner 压缩发现与筛选

默认 AI 阅读使用独立的压缩发现投影，而不是在现有全文索引上增加 L2–L4 过滤。发现投影只使用 Owner 元数据、L4、L3、L2 紧凑内容和 L1 `retrieval_description`；L1 blocks、完整文稿和 L0 留在完整读取或用户显式选择的全文补偿路径。

```text
规范记录当前版本 → 独立词法/向量发现投影及Owner水位
    → identity / lexical / dense各路线先折叠Owner
    → 跨路线融合Owner并保留实际命中
    → 每Owner选择少量相关且互补的筛选窗口
    → CE逐窗口排序，AI一次判断一批Owner（relevant / uncertain / irrelevant）
    → relevant Owner逐篇完整阅读并写note

发现索引不可用、覆盖不足或结果不足
    → 报告尚未搜索的全文范围
    → 等用户明确选择
    → 才执行L1 blocks/文稿等全文补偿召回
```

发现投影是可重建查找辅助，不能成为正文、证据或规范记录的替身。每个 Owner 的词法/向量水位独立记录 `indexed|pending|unavailable|stale|failed`，任一通道失败不会把整体报为已就绪；查询不回退到全文索引。旧 Owner 缺 L4 或 L1 检索说明缺失/为空时，`discovery-gaps` 只报告确定性缺口；非空说明是否充分仍由低等级模型完整阅读后判断。模型通过 MEM 版本提交补写并回读后，才以该 HEAD 调用 `rebuild-discovery`。本次审核的三个长期 Owner 均无正文结构缺口，因此没有生成内容修订，只逐 Owner 重建并核对词法/向量水位；真实升级/恢复和端到端调用链已验证。完整算法、关系边界、状态和失败处理见[Owner压缩发现、互补筛选包与全文补偿](design/OWNER_DISCOVERY_RETRIEVAL.md)。

AI围绕当前问题使用阅读工作流时，在下述基础检索外组织以下过程：

```text
接续目标、条件、已读理解与尝试结果（已有上下文够用就不重复读取）
程序核对协作开关与宿主能力 → 独立低成本reader或同RS单Agent
    → reader执行下方过程；主Agent只收有界reading-handoff笔记及固定出处
需要新依据 → 各层独立多路召回与RRF去重
    → 可选有界正文/必要定义 → 离线Cross-encoder和显式字段条件 → 保留原排名与缺口
    → 按固定正文块去重，AI候选只看正文和Owner；详细诊断留内部
standard/associative：AI判断直接用途或具体迁移理由 → 选Owner，后续跳过其余候选
    → 逐Owner读取已有完整文稿 → 图片/L0/Run保留引用，确有需要才展开
quick：逐条判断实际交付片段 → 只将判为有用的片段写入note，明确非全文
associative不足时：主Agent制定联想问题和关键词，reader按给定文本受限召回，不把线索当机制证据
reader提取相关公式/条件/实验/图示与出处 → 写连贯的简化完整文稿（问题演进/方法/证据/认识修正）并在同一任务自查补齐
跨Owner理解 → 保存固定底稿/来源的综合note；变化后重验
保存时程序检查引用 → 主Agent获得note后直接推进工作，不设固定复检轮次
...
阅读笔记同时有机器状态和可读正文：AI 消费程序组织好的 Markdown 交接；正文只承载对搜索问题有用的材料知识、条件、经验与局限，材料建议使用“可选的下一步建议”。会话问题和 reader 的决定、下一步保留在状态中，不重复拼入知识正文；交付缺口仍需明确返回。工作台首页统一选择最近问题、刷新笔记及控制模式，系统记忆不再另设阅读记录页。记忆摘要和笔记引用可跳转证据详情，不能把同一对象的全部记录自动说成支持证据。本地阅读副本的保存、版本和边界见 [AI 阅读手册](AI_READING.md#存储预算与恢复)。

按需补充：[三张索引表如何配合一次查询](MEMORY_STORAGE_EXPLAINED.md#3-索引到底存什么)——包括记录与条目的区别和语义查询。
...
对每个选中候选：
    ...
    通用MQ固定栅格图示随正文交付并计读取预算；Owner AI阅读先保留图号与引用，显式展开才交付图像
