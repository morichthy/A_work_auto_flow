...
归档后补记：最终validate仍为0错误；19个警告包含原8项和本次原样固定Markdown快照的11项相对链接提示。现行Skill入口不受影响，固定快照不改字节；见[最终回读](../runs/run-20260915t204918z-4af8af30e4e8/FINAL_REVIEW.md)。此前8项是归档前实际回执，保留原记录。

## 对话与机制说明整理（2026-09-16后续）

用户要求将本次问题和关键结果整理成完整叙述，并从CORE对应位置链接。复用现有 [MEMORY_STORAGE_EXPLAINED](../../../docs/MEMORY_STORAGE_EXPLAINED.md)，补齐持续工作目录/自主保存、record与commit及旧版、C10–C14真实演进、overview/document/manifest分工、文稿选择/组装、最新与有效的区分、文稿树trade-off、新Skill闭环和已完成验证。CORE第1/2/3/6节加入简要说明和定向链接，docs/README同步导航。

本次为说明文档维护，沿用已读现行文稿、Skill、真实manifest和源码依据，区分实现事实、历史例子、用户采纳的维护要求及有限验证结果。未改API、安装登记或冻结Run，未将本轮编辑追写入前轮固定快照；无需重跑软件/升级测试。

收口检查：已全文回读；CORE、说明手册和文档索引的152个本地链接/章节锚点检查通过；workspace validate为0错误、19个既有快照链接警告，未新增。独立只读审查确认当前指针/历史、文稿选择、必要阅读和未引用变化、索引独立状态等关键边界一致；同时清除根AGENTS的重复规则，并澄清RESEARCH_RECORDING中索引状态单列及明确要求的检索通道边界。上述审查是文档一致性检查，不是新的端到端软件验证。

本阶段完整成果已同步（范围：本次对话的机制说明、CORE对应摘要/链接及直接相关规则表述；AI文档一致性自查）。前轮固定Run保留原样，本补记记录后续说明维护。
