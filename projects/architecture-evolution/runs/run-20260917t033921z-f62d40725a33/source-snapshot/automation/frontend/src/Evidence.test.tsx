...
it("证据深链按ID加载单条详情并可跳转引用，搜索不请求全库state", async () => {
  ...
  call.mockImplementation(async (path, payload) =>
    path === "evidence/search"
      ...
      : {
          ...
          structured_content_markdown: "## 研究阶段\n\n结构化重复内容",
  ...
  expect(screen.getByText("复核不适用于所选版本")).toBeVisible();
  expect(screen.getByText("结构化重复内容")).not.toBeVisible();
  fireEvent.click(screen.getByText("结构化记录字段"));
  expect(screen.getByText("结构化重复内容")).toBeVisible();
