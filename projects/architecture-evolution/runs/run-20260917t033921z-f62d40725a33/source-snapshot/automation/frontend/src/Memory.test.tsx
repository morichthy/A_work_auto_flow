...
describe("统一记忆层级入口", () => {
  it("每层只有一个筛选和新建入口，其他对象未迁移的内容仍可找到", async () => {
    const records = Object.fromEntries(
      ["event", "narrative", "map", "overview", "document_section", "goal"].map(
        (kind) => [
          kind,
          {
            record_id: kind,
    ...
    expect(within(filters).getAllByRole("checkbox")).toHaveLength(6);
    expect(
      screen.queryByRole("heading", { name: "合成 goal" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "合成 document_section" }),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("显示辅助工作流记录"));
    expect(
      screen.getByRole("heading", { name: "合成 goal" }),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "清空类型" }));
    expect(
      screen.queryByRole("heading", { name: "合成 goal" }),
    ).not.toBeInTheDocument();
    expect(screen.getByLabelText("显示辅助工作流记录")).not.toBeChecked();
    fireEvent.click(screen.getByRole("button", { name: "全选类型" }));
