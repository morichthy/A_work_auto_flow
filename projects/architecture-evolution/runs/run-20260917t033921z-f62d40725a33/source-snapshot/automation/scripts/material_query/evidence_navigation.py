...
def render_content_citations(text, refs, titles):
    ...
    import re
    # Authored numbered bibliographies already define their own local numbering.
    # Their order can differ from sources/payload traversal, and chapters may
    # each restart at [1]. Preserve those numbers instead of silently attaching
    # a second, contradictory automatic numbered bibliography. Explicit saved
    # IDs still become exact-version links; unlocated refs remain in the page's
    # separate references panel and are never guessed into numbered prose.
    protected = r'(```[\s\S]*?```|`[^`\n]*`|\$\$[\s\S]*?\$\$|\$[^$\n]*\$|\\\[[\s\S]*?\\\]|\\\([^\n]*?\\\)|!?\[[^\]]*\]\([^\n]*?\)|https?://\S+)'
    parts = re.split(protected, text)
    plain = '\n'.join(parts[::2])
    authored = any(re.search(r'^\s*(?:-\s*)?\[\d+\]\s+[^\n]*' + re.escape(ref['id']) + r'(?![A-Za-z0-9_-])',
                             plain, re.MULTILINE) for ref in refs)
    if authored:
        by_id, missing = {}, set()
        for item in references:
            by_id.setdefault(item['ref']['id'], []).append(item)
        pattern = re.compile(r'(?<![A-Za-z0-9_-])(\[)?(' +
                             '|'.join(re.escape(key) for key in sorted(by_id,key=len,reverse=True)) +
                             r')(?![A-Za-z0-9_-])(?(1)\])(?:[ \t]+r(\d+))?')
        def fixed_link(match):
            candidates = by_id[match[2]]
            if match[3]:
                candidates = [item for item in candidates if item['ref'].get('revision') == int(match[3])]
            if len(candidates) != 1:
                missing.add(match[0])
                return match[0]
            item = candidates[0]
            revision = item['ref'].get('revision')
            label = '固定版本' + (' r' + str(revision) if revision is not None else '')
            return '['+label+']('+item['url']+')'
        for position in range(0,len(parts),2):
            parts[position] = pattern.sub(fixed_link, parts[position])
        rendered = ''.join(parts)
        if missing:
            rendered += '\n\n> 固定引用缺口：以下标注无法唯一匹配已登记的修订或位置，保留原文，未替换为其他版本：' + '；'.join(sorted(missing))
...
_PAYLOAD_LABELS.update({'stages':'研究阶段','situation':'问题背景','outcome':'阶段结果',
    'reason':'选择依据','cannot_infer':'不能据此推断','category':'类别','retry_conditions':'复试条件',
    'tested_scope':'已测试范围','problem_structure':'问题结构','prohibited':'不适用或禁止使用',
    'failure_modes':'失败模式','confidence':'置信说明','claim_id':'结论编号','statement':'结论内容',
    'uncertainty':'不确定性','knowledge_type':'认识类型','entry_type':'条目类型'})
_VALUE_LABELS = {'counterexample':'反例','execution_failure':'执行失败','observation':'观察',
                 'conclusion':'结论','hypothesis':'假设','recommendation':'建议'}
...
def readable_value(value):
    ...
    if isinstance(value, dict):
        ...
        return '\n\n'.join(('**'+_PAYLOAD_LABELS.get(key,key)+'**\n\n'+
                            (_VALUE_LABELS.get(item,item) if key in {'category','knowledge_type','entry_type'} and isinstance(item,str) else readable_value(item)))
...
def record_content(catalog, record, seen=None):
    ...
    for key,value in payload.items():
        ...
        rendered=(_VALUE_LABELS.get(value,value) if key in {'category','knowledge_type','entry_type'} and isinstance(value,str)
                  else readable_value(value))
...
def review_metadata(catalog, data):
    """Display saved confirmation separately from evidence validity.

    Reviews belong to the claim's Owner. Read only that Owner's canonical
    records and compare the saved content/claim bindings; do not hash original
    files or run the scientific dependency graph during a detail-page read.
    A current review of different content must never confirm a pinned version.
    """
    from memory.contracts import canonical_hash
    claims = data.get('payload', {}).get('claims', data.get('claims', []))
    if data.get('claim_id'):
        claims = [data]
    if not claims:
        return []
    reviews, incomplete = {}, False
    if data.get('record_id'):
        selected_claims = {claim['claim_id'] for claim in claims}
        for rid, row in catalog.records.items():
            if row.get('owner_id') != data['owner_id']:
                continue
            try:
                record = catalog.record(rid)
            except (ValueError, OSError, MemoryError, QueryError):
                # An inaccessible sibling must not break a readable detail, or
                # falsely establish that the selected claim has no review.
                incomplete = True
                continue
            if record.get('kind') == 'review':
                ...
                if cid not in selected_claims:
                    continue
                # The target claim is already authorized as part of the selected
                # record. Its binding is checked below, so remove only that
                # self-reference from the authorization walk (which otherwise
                # resolves native claims). Other review evidence stays checked.
                try:
                    catalog.authorize(dict(payload=record['payload'], sources=[
                        ref for ref in record['sources'] if not
                        (ref.get('target_kind') == 'claim' and ref.get('target_id') == cid)]))
                except (ValueError, OSError, MemoryError, QueryError):
                    incomplete = True
                    continue
                if cid in reviews:
                    raise ValueError('同一结论存在多个当前复核记录')
                reviews[cid] = record
    result = []
    for claim in claims:
        review = reviews.get(claim['claim_id'])
        payload = review['payload'] if review else claim.get('review', {})
        matches = None
        if review:
            bindings = [ref for ref in review['sources'] if ref.get('target_kind') == 'claim'
                        and ref.get('target_id') == claim['claim_id']]
            matches = (payload['target_content_hash'] == data.get('content_hash')
                       and len(bindings) == 1 and bindings[0].get('sha256') == canonical_hash(claim))
        result.append(dict(claim_id=claim['claim_id'], statement=claim.get('statement', ''),
                           state=payload.get('state', payload.get('status', 'unavailable' if incomplete else 'not-reviewed')),
                           content_matches=matches, scope=payload.get('scope') or claim.get('scope'),
                           reviewer=payload.get('reviewer'), reason=payload.get('reason'),
                           reviewed_at=review.get('updated_at') if review else payload.get('reviewed_at'),
                           evidence_validity='not_rechecked'))
    return result
...
def detail(service, raw):
    ...
    try:
        ...
        if registered:
            ...
            if (data.get('kind') in {'narrative','experience','overview','event','map'}
                    and data.get('body_markdown','').strip() and not locator):
                # A complete authored narrative is the primary reading surface.
                # Preserve the machine-oriented payload in a separate optional
                # section instead of repeating it immediately after the prose.
                structured_record = deepcopy(data)
                structured_record['body_markdown'] = ''
                structured_body, _ = record_content(catalog, structured_record)
                content_record['payload'] = {}
        ...
        if data.get('provenance_gap'):
            context['provenance_gap'] = data['provenance_gap']
        if data.get('payload',{}).get('missing_refs'):
            context['missing_refs'] = deepcopy(data['payload']['missing_refs'])
        if kind == 'file':
        ...
        if structured_body:
            structured_body = render_content_citations(structured_body, fixed_sources,
                {item['id']:item['title'] for item in links})[0]
        def decorate(rows):
            for section in rows:
        ...
        return dict(workspace_id=workspace_id(service.root), id=identity, owner_id=oid, kind=kind, title=title,
                    content_markdown=body, structured_content_markdown=structured_body,
                    tags=data.get('keywords', []), context=context, citation_references=citation_references,
                    ...
                    sections=sections, images=images, media=media, claim_reviews=review_metadata(catalog, data),
