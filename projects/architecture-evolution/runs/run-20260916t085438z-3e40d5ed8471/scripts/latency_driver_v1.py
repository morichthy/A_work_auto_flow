...
def invoke(state,action,raw,actor,profile=False):
    ...
    if profile:
        ...
        profiler.enable()
    stamp=time.perf_counter_ns()
    app=Coordinator(ROOT,access_owner_ids=[state['source_owner'],state['binding_owner']])
...
def prepare(args):
    ...
    state.update(session_id=template['session_id'],scope=scope,question=args.question)
